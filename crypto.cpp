#include "crypto.h"

#include <cryptopp/des.h>
#include <cryptopp/modes.h>
#include <iostream>

using namespace CryptoPP;

byte iv[] = {};

Crypto::Crypto()
{

}

bool Crypto::encrypt(std::string fname, std::string key)
{
    BlockTransformation *trans = NULL;


    byte bKey[DES_XEX3::KEYLENGTH];
    memcpy(bKey, key.c_str(), DES_XEX3::KEYLENGTH);

    try
    {
        trans = new DES_XEX3_Encryption(bKey, DES::KEYLENGTH);
        //CBC_Mode< DES_EDE3 >::Encryption e;
        //e.SetKeyWithIV(bKey, sizeof(iv), iv);
    }
    catch(const CryptoPP::Exception e)
    {
        std::cerr << e.what() << std::endl;
        return false;
    }
}

bool Crypto::decrypt(std::string fname, std::string key)
{

}
